    ...
    while(T--)
    {
        ...
        for(int i = 0; i < numOfPiece; ++i)
        {
            if(mark[i] == 0)
            {
                ...
                qsort(notimeout, indexOfNoTimeout, sizeof(notimeout[0]), cmp1);//对提交时间升序快速排序
                qsort(timeout, indexOfTimeout, sizeof(timeout[0]), cmp2);//对处理时间升序快速排序
                //先选未超时序列中提交时间最小的，若未超时序列为空，则渲已超时序列处理时间最小的
                indexOfMaxPriority = (indexOfNoTimeout > 0) ? notimeout[0] : timeout[0];
            }
        }
        mark[indexOfMaxPriority] = 1;//将该工件的访问数组置为1
        sequence[cnt++] = array[indexOfMaxPriority].n;//并记录当前序列所需信息
